...
interface ZoomImageProps extends WithStyles<typeof styles> {
  alt: string;
  src: string;
  theme: any;
  zoomedImgProps?: React.HTMLProps<HTMLImageElement>;
  className?: string;
}

function ZoomImage(props: ZoomImageProps): JSX.Element {
  const { alt, src, zoomedImgProps, classes, className, ...rest } = props;
  const [zoomedIn, setZoomedIn] = useState(false);
  const [scrollbarSize, setScrollbarSize] = useState<number | null>(null);
